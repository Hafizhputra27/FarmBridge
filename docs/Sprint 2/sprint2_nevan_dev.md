...
- [ ] Konfigurasi Supabase Anonymous Auth (`auth.signInAnonymously()`) sebagai mekanisme sesi satu-satunya — tidak ada email/password sama sekali
- [ ] RLS: `farmer_profiles`, `buyer_profiles`, `listings`, `trust_metrics`, `buyer_metrics`, `price_reference_data` → **public read**, write terbatas ke pemilik (farmer/buyer terkait)
- [ ] RLS: `negotiations`, `negotiation_messages`, `transactions`, `recurring_orders` → **participant-only** (`auth.uid() = buyer_id or auth.uid() = farmer_id`)
- [ ] `users` → self read/write only
- [ ] Storage bucket `listing-photos`, policy upload hanya oleh role `farmer` pemilik listing, read publik
- [ ] Pastikan `service_role` key TIDAK PERNAH dipakai di client — hanya di Edge Functions
...
- [ ] `create extension if not exists pg_cron;` — kerangka job saja, logic bisnis (auto-expire 6 jam FG-24 di Sprint 6, trigger H-1 recurring di sprint lanjutan) dikerjakan di ticket fitur masing-masing
- [ ] Pastikan tiap eksekusi job pg_cron mencatat log terpisah yang bisa ditinjau (PRD §12 Observability)
...
## Definition of Done
- [ ] RLS teruji: buyer A tidak bisa akses `negotiations`/`transactions` milik buyer B — **cara cek**: login sebagai buyer A via Postman/curl dengan JWT-nya, `GET` row milik buyer B, harus dapat array kosong
- [ ] RLS teruji: `trust_metrics`/`buyer_metrics`/`farmer_profiles`/`buyer_profiles` bisa dibaca lintas user (public read) tapi tidak bisa ditulis selain pemilik — **cara cek**: coba `PATCH` profil orang lain dari akun berbeda, harus ditolak
- [ ] Upload foto ke `listing-photos` hanya berhasil dari role `farmer` — **cara cek**: coba upload dari akun `buyer`, harus ditolak
- [ ] Pesan baru di `negotiation_messages` diterima via Realtime tanpa polling, < 2 detik (PRD §12 NFR) — **cara cek**: subscribe dari 2 client, insert row baru, ukur waktu event masuk
- [ ] `pg_cron` extension aktif, tabel `cron.job` bisa didaftari — **cara cek**: `select * from cron.job;` tidak error
